...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def forward(
        ...
        """
        encoder_out, z_encoder_out = self.encoder(
            src_tokens,
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        # add reasoning layer # reasoning layer 13
        self.layers.extend( 
            [StrGNNLayer(args).float()]
        )
    ...
    def forward(
        ...
        segments = segments.eq(50261) # Obtain T/F index; B x T x C
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.encoder_layerdrop):
                if i == len(self.layers) - 1: # HierGNN
                    gnn_out, gnn_int_m, gnn_r_prob, sent_padding_mask, sent_lens = layer(x, encoder_padding_mask, segments)
                else:
        ...
        sent_out = EncoderOut(
            encoder_out=gnn_out,  # M x B x C
            encoder_padding_mask=sent_padding_mask,  # B x M
            encoder_embedding=encoder_embedding,  # B x T x C
            encoder_states=encoder_states,  # List[T x B x C]
        )

        return token_out, sent_out, gnn_int_m, gnn_r_prob
    ...
    def upgrade_state_dict_named(self, state_dict, name):
        ...
        for i in range(self.num_layers - 1): # ignore HierGNN layers in Transformer Encoder
            # update layer norms
