...
class BARTHubInterface(nn.Module):
    ...
    def generate(self, tokens: List[torch.LongTensor], beam: int = 5, verbose: bool = False, **kwargs) -> torch.LongTensor:
        ...
        translations = self.task.inference_step(
            ...
            bos_token=self.task.source_dictionary.bos() # add bos_token=self.task.source_dictionary.bos() according to https://github.com/pytorch/fairseq/issues/1971#ref-issue-595576318
        )
    ...
    def extract_reasoning_chain(self, sentences: torch.LongTensor, return_all_hiddens: bool = False) -> torch.Tensor:
        tokens = [self.encode(sentence) for sentence in sentences]
        sample = self._build_sample(tokens)
        ...
        encoder_out, z_encoder_out, gnn_int, r_probs = self.model.forward_encoder(**encoder_input)
        
        return encoder_out, z_encoder_out, gnn_int, r_probs
